...
dt_max = min([2*(dx/uT)^2 , 0.25*dx*dx/nu , 4.0*nu/uT^2]);
%% CFL condition
dt = dx/uT;
CFL_no = uT*dt/dx;
% dt = 1e-6;
...
%% Time integration
n = 0;
%% Starting the solution with a Euler explicit time advancement
...
%         vt(j,i) = v(j,i) + dt*(convection(j,i) + diffusion(j,i));
...
% u_old = u;
% v_old = v;
%% Continuation of the soliution with Adams-Bashforth time advancement
